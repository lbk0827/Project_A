#!/usr/bin/env python3
"""Create CharacterCards.xlsx: the heroine's card set + the effect keyword table.

- character_cards: one row per card (cost, type, structured effects, keywords,
  inspiration payload, starting-deck copies).
- card_effects: keyword glossary parsed from Docs/Card_Effects_Glossary.md so the
  table stays in sync with the design doc.

Damage percents are of attack_power, shield percents are of defense_power. This
is authoring data only; the combat runtime that executes these is a later step.
"""
...
CARD_FIELDS = ["character", "id", "display_name", "cost", "card_type", "text", "copies", "keywords[]", "effects", "inspiration"]
CARD_TYPES = [None, "string", "key,string", "string", "int", "string", "string", "int", "string", "json", "json,null"]
CARD_WIDTHS = {"B": 12, "C": 20, "D": 18, "E": 8, "F": 12, "G": 46, "H": 8, "I": 16, "J": 52, "K": 40}
...
def build_character_cards(sheet) -> None:
	...
	for character, card_id, name, cost, card_type, text, copies, keywords, effects, inspiration in CHARACTER_CARDS:
		sheet.append([
			None, character, card_id, name, cost, card_type, text, copies,
			",".join(keywords), effects, inspiration,
		])
...
def parse_glossary(glossary_path: Path) -> list[tuple[str, str, str]]:
	...
	for line in glossary_path.read_text(encoding="utf-8").splitlines():
		...
		if keyword in ("키워드", "") or set(keyword) <= {"-", " "}:
			continue
